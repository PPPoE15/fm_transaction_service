...
class ListCategories(BaseQueries):
    ...
    def _apply_filters(
        ...
        if filter_params.category_type:
            stmt = stmt.filter(
                db_models.Transaction.transaction_type == filter_params.category_type,
            )
        if filter_params.below_money_sum:
            stmt = stmt.filter(
                db_models.Transaction.transaction_date <= filter_params.below_money_sum,
            )
        if filter_params.above_money_sum:
            stmt = stmt.filter(
                db_models.Transaction.transaction_date >= filter_params.above_money_sum,
            )
        return stmt
